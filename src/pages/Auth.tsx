...
// OAuth helper function to use Capacitor Browser for in-app OAuth flows
async function signInWithOAuthCapacitor(provider: 'google' | 'apple') {
  try {
    ...
    if (data.url) {
      // Use Capacitor Browser to open the OAuth URL in-app
      await Browser.open({
        url: data.url,
        windowName: '_blank',
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pt-[clamp(0.75rem,2vh,2rem)] touch-manipulation">
            ...
            {step === 'method' && (
              <div className="flex flex-col items-center justify-center h-full gap-6 animate-fade-in pointer-events-auto">
                ...
                <button
                  type="button"
                  onClick={() => signInWithOAuthCapacitor('google')}
                ...
                <button
                  type="button"
                  onClick={() => signInWithOAuthCapacitor('apple')}
