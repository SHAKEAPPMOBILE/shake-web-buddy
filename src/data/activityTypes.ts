...
// getNextOccurrenceDate below and findOrCreateOpenGroup in lib/activityGroups).
// Formatting that noon timestamp's time-of-day directly is always wrong for
// these plans; use the activity type's real, fixed start time instead. Real
// (non-auto-generated) plans use their own genuine scheduled_for time.
export function getActivityTimeString(
  activityType: string,
  isAutoGenerated: boolean | null | undefined,
  scheduledForDate: Date | null,
  formatTime: (d: Date) => string
): string | null {
  if (isAutoGenerated) return ACTIVITY_START_TIMES[activityType] ?? null;
  if (!scheduledForDate) return null;
  return formatTime(scheduledForDate);
}
