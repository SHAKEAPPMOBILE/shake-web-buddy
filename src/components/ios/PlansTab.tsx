...
export function PlansTab({ onChatViewChange, pendingPaidActivityId, onPendingPaidActivityHandled, pendingNewPlanId, onPendingNewPlanHandled, onOpenEvents, onJoinActivity, onConfirmActivity }: PlansTabProps = {}) {
      ...
      const [enrichedReal, enrichedCarousel, enrichedCity, enrichedDiscovery] = await Promise.all([
        ...
        (async () => {
          ...
          const realRowsRes = realIds.length
            ? await supabase.from("user_activities").select("id, user_id, is_auto_generated, promo_video_url, promo_image_url, description").in("id", realIds)
            : { data: [] as any[] };
  ...
  // No DB round-trip: purely derived from state already loaded, and never
  // attached to any real user_activities row (there may not be one).
  const standingOpenGroupCards = useMemo(() => {
    if (!selectedCity) return [];
    ...
    return ACTIVITY_TYPES.filter((type) => {
      ...
      const dayLabel = getActivityDay(type.id);
      return {
        id: `standing-open-${type.id}-${normCity}`,
        user_id: undefined,
        activity_type: type.id,
        city: selectedCity,
        scheduled_for: getNextOccurrenceDate(type.id).toISOString(),
        is_active: true,
        is_auto_generated: true,
        note: dayLabel ? `This ${dayLabel}` : null,
        creator_name: "Open group",
    ...
    return (
      <PlanGroupChatView
        activity={{
          ...
          promo_image_url: selectedPlan.promo_image_url ?? null,
        }}
        onBack={handleBackFromChat}
      />
    );
