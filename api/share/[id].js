...
function buildOgHtml({ id, activityType, city, participantCount, note, creatorAvatarUrl, promoImageUrl, promoVideoThumbnailUrl }) {
  ...
  // The plan's own media beats everything else — it's what the organizer
  // actually chose to represent this specific plan. A photo upload wins
  // over a video's captured frame when somehow both are set (shouldn't
  // happen — a plan has one hero media item — but photo is the more
  // deliberate choice if it does); either beats the creator's avatar,
  // which beats a generic per-activity-type icon, which beats the plain
  // SHAKE logo.
  const image = resolveAvatarUrl(promoImageUrl) || resolveAvatarUrl(promoVideoThumbnailUrl) || resolveAvatarUrl(creatorAvatarUrl) || meta.image;
...
export default async function handler(req, res) {
  ...
  try {
    const actRes = await fetch(
      `${SUPABASE_URL}/rest/v1/user_activities?id=eq.${encodeURIComponent(id)}&is_active=eq.true&select=id,activity_type,city,note,user_id,promo_image_url,promo_video_thumbnail_url&limit=1`,
    ...
    const html = buildOgHtml({
      ...
      promoVideoThumbnailUrl: activity.promo_video_thumbnail_url,
    });
